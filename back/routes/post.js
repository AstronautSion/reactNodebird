...
const { isLoggedIn, hasPostIn } = require('./middleware');
...
const upload = multer({
  storage: multer.diskStorage({
    ...
    filename(req, file, done) {
      ...
      const basename = path.basename(file.originalname, ext); // 1.png, ext===.png, basename===1
...
router.post('/', isLoggedIn, upload.none(), async (req, res, next) => { // POST /api/post
  try {
    ...
    const newPost = await db.Post.create({
      content: req.body.content, // ex) '1 파이팅 #구독 #좋아요 눌러주세요'
      UserId: req.user.id,
    });
    if (hashtags) {
